...
class TechnicalAnalyst:
    ...
    def start_loop(self, interval=600):
        """Audit all newly flagged coin picks every 10 minutes."""
        logger.info(f"🏎️ Technical Analyst is online. Heartbeat: {interval}s.")
        while True:
            # In a real build, we'd fetch the current 'Promoted' list from Notion
            # for the skeleton, we audit a trial set.
            self.run_technical_audit("BTCUSDT")
            time.sleep(interval)

if __name__ == "__main__":
    analyst = TechnicalAnalyst()
    analyst.run_technical_audit("BTCUSDT")
